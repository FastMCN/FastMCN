...
class GetFt():
    ...
    def get_data_fast(self, data_set):
        word_code, n_words, names, label = self.get_data(data_set)
        sparse_idx = []
        sparse_value = []
        n = 0
        for i, w in enumerate(n_words):
            ...
            sparse_value += [1] * w
            n += w
        n_words_tensor = torch.sparse.LongTensor(torch.tensor(sparse_idx).T,
...
config_name = sys.argv[1] if len(sys.argv) > 1 else "ncbi_char_transformer_512"
